...
public class BarToolHandler implements ToolHandler {
    ...
    public ToolResult execute(ToolContext context, Map<String, Object> arguments, ToolOutputChannel channel) {
        ...
        if (!s3Service.isConfigured()) {
            return new ToolResult(JacksonUtils.serialize("S3存储服务未配置，无法生成柱状图。"));
        }
        ...
        try {
            ...
            // 返回错误消息
            return new ToolResult("生成柱状图或上传到S3失败: " + e.getMessage());
        }
    ...
    public Map<String, Object> getParameters() {
        ...
        xAxisParam.put("type", "string");
        xAxisParam.put("description", "横坐标的标签（可选）");
        properties.put("xAxisTag", xAxisTag);

        Map<String, Object> yAxisTag = new HashMap<>();
        xAxisParam.put("type", "string");
        xAxisParam.put("description", "纵坐标的标签（可选）");
